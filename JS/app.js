...
async function postLike(postID, driver, end_cursor_like = null) {
  ...
  if (end_cursor_like == null) {
    ...
    await driver
      ...
      .then((text) => {
        ...
        liker.forEach((fol) => {
          fs.appendFile(`./${postID}.txt`, `${fol.node.username}\n`, (err) => {
            if (err) {
              console.log(err);
            }
          });
...
// the main function
async function getLike(target, userName, password, end_cursor_like = null) {
  let postID = [];
  let postNo;
  // get random no. from 0-5000
  function random(max) {
  ...
  for (let i = 0; i < postNo; i++) {
    fs.appendFileSync("./post.txt", `${postID[i]}\n`, (err) => {
      if (err) {
        console.log(err);
      }
    });
  ...
  // postID.forEach((post) => {
  //   fs.appendFile("./post.txt", `${post}\n`, (err) => {
  //     if (err) {
  //       console.log(err);
  //     }
  //   });
  // });
  postID.asyncforEach((shortcode) => {
    await postLike(shortcode, driver);
